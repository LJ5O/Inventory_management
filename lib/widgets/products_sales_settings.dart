...
class ProductsSalesSettings extends StatelessWidget{
  ...
  Widget build(BuildContext context) {
    ...
    return FutureBuilder<int>(
      future: storage.getInt('low_stock_threshold'),  // Getting default value for low availability warning
      builder: (BuildContext context, AsyncSnapshot<int> snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
          return const CircularProgressIndicator();  // Afficher un loader pendant le chargement
        } else {
          int initialValue = snapshot.data ?? 0;
          TextEditingController textController = TextEditingController(text: initialValue.toString());
          ...
          return Column(
            ...
            children: [
              ...
              const Text(
                "Ce bouton permet de remettre tous les compteurs du marché à 0, et retirera les valeurs de ce qu'il reste en stock.",
                textAlign: TextAlign.center,
              ),
              ElevatedButton(
                onPressed: () {
                  resetSalesProducts(true);
                  Navigator.pop(context);
                },
                style: ElevatedButton.styleFrom(backgroundColor: Colors.green),
                child: const Text(
                  "Remettre les ventes à zéro, et retirer cela du stock",
                  style: TextStyle(color: Colors.white),
                ),
              ),
              const Padding(padding: EdgeInsets.all(20.0)),
              const Text(
                "Ce bouton va remettre tous les compteurs du marché à 0, SANS retirer les valeurs du stock.",
                textAlign: TextAlign.center,
              ),
              ElevatedButton(
                onPressed: () {
                  resetSalesProducts(false);
                  Navigator.pop(context);
                },
                style: ElevatedButton.styleFrom(backgroundColor: Colors.red),
                child: const Text(
                  "Remettre les ventes à zéro, sans retirer du stock",
                  style: TextStyle(color: Colors.white),
                ),
              ),
            ],
